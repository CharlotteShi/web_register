...
public class doctorDao {
	...
	public List<Doctor> sdepartment(String department) throws Exception//按科室查询医生返回一个列表
	{
		...
		while(rs.next()){
			...
			g.setA20161128(rs.getInt("a20161128"));
			g.setA20161129(rs.getInt("a20161129"));
			g.setA20161130(rs.getInt("a20161130"));
			result.add(g);
			}
	...
	public void updateDoctor(Doctor g) throws SQLException{//更新医生挂号信息
		Connection conn=DBUtil.getConnection();
		String sql=""+" update Doctor"+
		" set a20161128=?,a20161129=?,a20161130=? "+
		" where id=? ";
		
		PreparedStatement ptmt=conn.prepareStatement(sql);
		//这个语句会把sql语句加载到驱动程序中，但不直接执行，而是当调用execute()方法时使用
     	ptmt.setInt(1, g.getA20161128());
     	ptmt.setInt(2, g.getA20161129());
     	ptmt.setInt(3, g.getA20161130());
